	...
	#endif
	{
		...
		if (flash_type > 0) {
			...
			if (total_rom_size > flash_sram_area) {
				...
				strmerge(str,"Jagoomba Color ", VERSION);
				drawtext(18,str,0);
				drawtext(19,"by Flubba, Dwedit, Jaga, LK",0);
				while (1) waitframe();
			}
		...
		if (i == 0)
		{
			...
			drawtext( 3,"You can create a compilation",0);
			drawtext( 4,"     using Goomba Front.",0);
#if FLASHCART
			drawtext( 9,"This build of Jagoomba Color",0);
